...
# ---------------------------------------------------------------------------
# Analyse des prix (rapport LLM, onglet "Analyse des prix")
# ---------------------------------------------------------------------------
@dataclass
class PriceAnalysisLLMConfig:
    # Texte seul (pas d'image) : Hugging Face suffit largement ici, pas
    # besoin de Gemini/Claude qui sont reserves a la vision multimodale.
    provider: str = "huggingface"       # "huggingface" / "ollama" / "template" (force le fallback local)
    model: str = "meta-llama/Llama-3.1-8B-Instruct"  # tres largement disponible chez les
                                                       # providers HF (Cerebras, Groq, Together,
                                                       # Fireworks, Novita, SambaNova...)
    api_key_env: str = "HF_TOKEN"        # cle gratuite : https://huggingface.co/settings/tokens
    ollama_model: str = "llama3.2"       # repli local si HF indisponible/hors quota
    max_tokens: int = 1200

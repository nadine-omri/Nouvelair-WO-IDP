...
interface SidebarProps {
  ...
  /** Onglets nécessitant un document sélectionné (grisés tant qu'aucun n'est chargé). */
  docRequiredTabs?: readonly string[];
  hasDocs?: boolean;
}
...
function NavButton({
  ...
  return (
    <button
      type="button"
      onClick={() => !disabled && onNavigate?.(tabKey)}
      disabled={disabled}
...
export default function Sidebar({
  ...
  return (
    <aside className="flex h-full w-full flex-col gap-5 overflow-y-auto scrollbar-thin p-5">
      <div className="flex items-center gap-3 border-b border-violet-100 pb-5">
        ...
        <div>
          <p className="font-display text-sm font-bold text-violet-950">Sabena IDP</p>
          <p className="text-[11px] text-violet-400">Gestion intelligente des WO</p>
        </div>
      </div>

      <nav className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <p className="px-2 pb-1 text-[10px] font-semibold uppercase tracking-wider text-violet-400">
            Vue d&apos;ensemble
          </p>
          {(
            [
              ["dashboard", "⌂", "Dashboard"],
              ["search", "⌕", "Historique des WO"],
              ["price-analysis", "◈", "Analyse des prix (IA)"],
            ] as const
          ).map(([key, icon, label]) => (
            <NavButton
              key={key}
              ...
              disabled={docRequiredTabs.includes(key) && !hasDocs}
            />
          ))}
        </div>
      ...
      <div>
        ...
        <div
          ...
          className={`group cursor-pointer rounded-2xl border-2 border-dashed p-6 text-center transition-all duration-300 ${
            dragOver
        ...
        >
          ...
          />
          <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-violet-100 text-violet-600 transition-transform duration-300 group-hover:scale-110">
          ...
          <p className="mt-1 text-xs text-violet-400">
            Images ou PDF scannés — plusieurs fichiers acceptés
          </p>
        </div>
          ...
          <div className="animate-fade-in rounded-xl bg-violet-50 px-3 py-2.5 text-xs text-violet-700 ring-1 ring-violet-100">
            <div className="mb-1.5 flex items-center justify-between">
              <span className="truncate font-medium">
                {progress.currentName || "Finalisation…"}
              </span>

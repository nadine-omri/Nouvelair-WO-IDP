"use client";

import { useState } from "react";
import {
  FIELD_LABELS,
  MaterialRow,
  SORT_LABELS,
  WorkOrderRecord,
  WorkOrderSortKey,
  workOrderToPipelineResult,
} from "@/lib/types";
import { searchWorkOrders, updateWorkOrderMaterials } from "@/lib/api";
import {
  SEARCH_EXPORT_COLUMNS,
  downloadCSV,
  downloadTablePdf,
  workOrdersToCsvRows,
  workOrdersToPdfRows,
} from "@/lib/export";
import MaterialsTab from "@/components/tabs/MaterialsTab";
import EngineBadge from "@/components/EngineBadge";
import ConfidenceBar from "@/components/ConfidenceBar";
import ExtractionPreviewTab from "@/components/tabs/ExtractionPreviewTab";
import TechnicalInfoTab from "@/components/tabs/TechnicalInfoTab";

const SORT_KEYS: WorkOrderSortKey[] = [
  "created_at",
  "date",
  "order_number",
  "ac_registration",
  "global_confidence_score",
];

type DetailView = "summary" | "data" | "technical";

export default function SearchTab({ onDemoFallback }: { onDemoFallback: () => void }) {
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [acRegistration, setAcRegistration] = useState("");
  const [query, setQuery] = useState("");
  const [sortBy, setSortBy] = useState<WorkOrderSortKey>("created_at");
  const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  const [loading, setLoading] = useState(false);
  const [searched, setSearched] = useState(false);
  const [results, setResults] = useState<WorkOrderRecord[] | null>(null);
  const [expanded, setExpanded] = useState<number | null>(null);
  const [detailView, setDetailView] = useState<DetailView>("summary");
  const [savingId, setSavingId] = useState<number | null>(null);

  const runSearch = async (overrides?: { sortBy?: WorkOrderSortKey; sortDir?: "asc" | "desc" }) => {
    setLoading(true);
    try {
      const res = await searchWorkOrders(
        {
          dateFrom: toFr(dateFrom),
          dateTo: toFr(dateTo),
          acRegistration,
          q: query,
          sortBy: overrides?.sortBy ?? sortBy,
          sortDir: overrides?.sortDir ?? sortDir,
        },
        onDemoFallback
      );
      setResults(res);
      setSearched(true);
    } finally {
      setLoading(false);
    }
  };

  const changeSort = (key: WorkOrderSortKey) => {
    setSortBy(key);
    if (searched) runSearch({ sortBy: key });
  };

  const toggleDir = () => {
    const next = sortDir === "asc" ? "desc" : "asc";
    setSortDir(next);
    if (searched) runSearch({ sortDir: next });
  };

  const handleMaterialsChange = (woId: number, materials: MaterialRow[]) => {
    setResults((prev) =>
      prev
        ? prev.map((r) =>
            r.id === woId ? { ...r, materials: materials as WorkOrderRecord["materials"] } : r
          )
        : prev
    );
  };

  const saveMaterials = async (wo: WorkOrderRecord) => {
    setSavingId(wo.id);
    try {
      await updateWorkOrderMaterials(wo.id, wo.materials, onDemoFallback);
    } finally {
      setSavingId(null);
    }
  };

  const toggleExpand = (id: number) => {
    if (expanded === id) {
      setExpanded(null);
    } else {
      setExpanded(id);
      setDetailView("summary");
    }
  };

  return (
    <div className="animate-fade-up flex flex-col gap-6">
      <div>
        <h3 className="text-base font-semibold text-violet-900">
          Recherche dans la base des ordres de travail
        </h3>
        <p className="text-xs text-violet-400">
          Base persistante : tous les CWO traités restent ici, y compris après fermeture de
          l&apos;application. Filtre par date, immatriculation ou texte libre, et trie les résultats.
        </p>
      </div>

      <div className="glass grid grid-cols-1 gap-4 rounded-2xl p-5 shadow-soft sm:grid-cols-2 lg:grid-cols-4">
        <DateField label="Date début" value={dateFrom} onChange={setDateFrom} />
        <DateField label="Date fin" value={dateTo} onChange={setDateTo} />
        <TextField
          label="Immatriculation"
          value={acRegistration}
          onChange={setAcRegistration}
          placeholder="TS-IMB"
          mono
        />
        <TextField
          label="Recherche libre"
          value={query}
          onChange={setQuery}
          placeholder="N° d'ordre, client, travaux, fichier…"
        />
        <div className="sm:col-span-2 lg:col-span-4 flex flex-wrap items-end gap-3">
          <button
            onClick={() => runSearch()}
            disabled={loading}
            className="shine flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 px-5 py-2.5 text-sm font-semibold text-white shadow-glow transition-all hover:from-violet-700 hover:to-violet-600 disabled:opacity-60"
          >
            {loading ? (
              <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-white/40 border-t-white" />
            ) : (
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
                <circle cx="11" cy="11" r="7" stroke="currentColor" strokeWidth="2" />
                <path d="M21 21l-4.3-4.3" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
              </svg>
            )}
            Rechercher
          </button>

          <div className="flex items-center gap-2">
            <label className="text-xs font-semibold text-violet-500">Trier par</label>
            <select
              value={sortBy}
              onChange={(e) => changeSort(e.target.value as WorkOrderSortKey)}
              className="rounded-xl border border-violet-100 bg-white/80 px-3 py-2 text-sm font-medium text-violet-900 shadow-sm transition-all focus:border-violet-400 focus:outline-none focus:ring-4 focus:ring-violet-100"
            >
              {SORT_KEYS.map((k) => (
                <option key={k} value={k}>
                  {SORT_LABELS[k]}
                </option>
              ))}
            </select>
            <button
              onClick={toggleDir}
              title={sortDir === "asc" ? "Croissant" : "Décroissant"}
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-violet-100 bg-white/80 text-violet-600 shadow-sm transition-all hover:bg-violet-50"
            >
              <svg
                width="15"
                height="15"
                viewBox="0 0 24 24"
                fill="none"
                className={`transition-transform duration-300 ${sortDir === "asc" ? "rotate-180" : ""}`}
              >
                <path d="M12 4v16m0 0l-5-5m5 5l5-5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
          </div>
        </div>
      </div>

      {results !== null && (
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs font-medium text-violet-400">
              {results.length} ordre{results.length > 1 ? "s" : ""} de travail trouvé
              {results.length > 1 ? "s" : ""} — trié par {SORT_LABELS[sortBy].toLowerCase()} (
              {sortDir === "asc" ? "croissant" : "décroissant"})
            </p>
            {results.length > 0 && (
              <div className="flex items-center gap-2">
                <button
                  onClick={() =>
                    downloadCSV("recherche_cwo", SEARCH_EXPORT_COLUMNS, workOrdersToCsvRows(results))
                  }
                  className="flex items-center gap-1.5 rounded-lg border border-violet-200 bg-white/70 px-3 py-1.5 text-xs font-semibold text-violet-700 transition-all hover:bg-violet-50"
                >
                  📄 CSV
                </button>
                <button
                  onClick={() =>
                    downloadTablePdf(
                      "recherche_cwo",
                      "Recherche — Ordres Client (CWO)",
                      `${results.length} résultat(s)`,
                      ["ID", "N° d'ordre", "Date", "Type avion", "Immat.", "Client", "MH requis"],
                      workOrdersToPdfRows(results)
                    )
                  }
                  className="flex items-center gap-1.5 rounded-lg border border-violet-200 bg-white/70 px-3 py-1.5 text-xs font-semibold text-violet-700 transition-all hover:bg-violet-50"
                >
                  🧾 PDF
                </button>
              </div>
            )}
          </div>

          {results.length === 0 ? (
            <div className="glass rounded-2xl p-8 text-center text-sm text-violet-400 shadow-soft">
              Aucun résultat pour ces critères.
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {results.map((wo) => (
                <div key={wo.id} className="glass overflow-hidden rounded-2xl shadow-soft">
                  <button
                    onClick={() => toggleExpand(wo.id)}
                    className="flex w-full flex-wrap items-center justify-between gap-3 px-5 py-4 text-left transition-colors hover:bg-violet-50/40"
                  >
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-1.5">
                      <span className="font-mono text-sm font-semibold text-violet-900">
                        CWO {wo.order_number || "—"}
                      </span>
                      <span className="text-xs text-violet-500">{wo.date || "—"}</span>
                      <span className="text-xs text-violet-500">{wo.lieu_place || "—"}</span>
                      <span className="rounded-full bg-violet-100 px-2.5 py-0.5 font-mono text-xs font-medium text-violet-700">
                        {wo.ac_registration || "—"}
                      </span>
                      <span className="text-xs text-violet-400">{wo.airline_customer || "—"}</span>
                      {wo.page_count != null && wo.page_count > 1 && (
                        <span className="rounded-full bg-sky-50 px-2 py-0.5 text-[11px] font-medium text-sky-700 ring-1 ring-sky-200">
                          page {wo.page_index}/{wo.page_count} — {wo.source_file}
                        </span>
                      )}
                      {wo.engine_used && <EngineBadge engine={wo.engine_used} />}
                      {wo.global_confidence_score != null && (
                        <span className="flex items-center gap-1.5">
                          <span className="w-14">
                            <ConfidenceBar value={wo.global_confidence_score * 100} size="sm" />
                          </span>
                          <span className="text-xs tabular-nums text-violet-500">
                            {Math.round(wo.global_confidence_score * 100)}%
                          </span>
                        </span>
                      )}
                      {wo.requires_review && (
                        <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-200">
                          À vérifier
                        </span>
                      )}
                    </div>
                    <svg
                      className={`h-4 w-4 shrink-0 text-violet-400 transition-transform duration-300 ${
                        expanded === wo.id ? "rotate-180" : ""
                      }`}
                      viewBox="0 0 24 24"
                      fill="none"
                    >
                      <path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
                    </svg>
                  </button>

                  {expanded === wo.id && (
                    <div className="animate-fade-up border-t border-violet-100 px-5 py-5">
                      <div className="mb-4 flex gap-1 overflow-x-auto border-b border-violet-100">
                        {(
                          [
                            ["summary", "Résumé & matériaux"],
                            ["data", "Données extraites complètes"],
                            ["technical", "Informations techniques"],
                          ] as [DetailView, string][]
                        ).map(([key, label]) => (
                          <button
                            key={key}
                            onClick={() => setDetailView(key)}
                            className={`relative shrink-0 whitespace-nowrap px-3.5 py-2.5 text-xs font-semibold transition-colors ${
                              detailView === key
                                ? "text-violet-700"
                                : "text-violet-400 hover:text-violet-600"
                            }`}
                          >
                            {label}
                            {detailView === key && (
                              <span className="absolute inset-x-1 -bottom-px h-0.5 rounded-full bg-gradient-to-r from-violet-600 to-violet-400" />
                            )}
                          </button>
                        ))}
                      </div>

                      {detailView === "summary" && (
                        <>
                          <div className="mb-5 grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                            <Info label={FIELD_LABELS.work_required} value={wo.work_required} full />
                            <Info label={FIELD_LABELS.ac_type} value={wo.ac_type} />
                            <Info label={FIELD_LABELS.required_mh} value={wo.required_mh} />
                            <Info label={FIELD_LABELS.customer_rep_name} value={wo.customer_rep_name} />
                            <Info label={FIELD_LABELS.customer_rep_date} value={wo.customer_rep_date} />
                          </div>

                          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-violet-500">
                            Matériaux vendus
                          </p>
                          <MaterialsTab
                            materials={wo.materials}
                            onChange={(m) => handleMaterialsChange(wo.id, m)}
                          />
                          <button
                            onClick={() => saveMaterials(wo)}
                            disabled={savingId === wo.id}
                            className="mt-3 flex items-center gap-2 rounded-xl border border-violet-200 bg-white/70 px-4 py-2 text-xs font-semibold text-violet-700 transition-all hover:bg-violet-50 disabled:opacity-60"
                          >
                            {savingId === wo.id ? (
                              <span className="h-3 w-3 animate-spin rounded-full border-2 border-violet-300 border-t-violet-600" />
                            ) : (
                              "💾"
                            )}
                            Enregistrer les matériaux dans la base
                          </button>
                        </>
                      )}

                      {detailView === "data" &&
                        (wo.extraction_fields && Object.keys(wo.extraction_fields).length > 0 ? (
                          <ExtractionPreviewTab result={workOrderToPipelineResult(wo)} />
                        ) : (
                          <EmptyTechnical />
                        ))}

                      {detailView === "technical" &&
                        (wo.engine_used ? (
                          <TechnicalInfoTab
                            result={workOrderToPipelineResult(wo)}
                            sourceFile={wo.source_file ?? undefined}
                            pageIndex={wo.page_index ?? undefined}
                            pageCount={wo.page_count ?? undefined}
                          />
                        ) : (
                          <EmptyTechnical />
                        ))}
                    </div>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>
      )}
    </div>
  );
}

function EmptyTechnical() {
  return (
    <div className="rounded-xl bg-violet-50/60 p-6 text-center text-xs text-violet-400">
      Aucune métadonnée technique disponible pour ce CWO (enregistré avant cette mise à jour, ou
      créé manuellement).
    </div>
  );
}

function toFr(isoDate: string): string {
  if (!isoDate) return "";
  const [yyyy, mm, dd] = isoDate.split("-");
  return yyyy && mm && dd ? `${dd}/${mm}/${yyyy}` : "";
}

function DateField({
  label,
  value,
  onChange,
}: {
  label: string;
  value: string;
  onChange: (v: string) => void;
}) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-semibold text-violet-500">{label}</label>
      <input
        type="date"
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full rounded-xl border border-violet-100 bg-white/80 px-3.5 py-2.5 text-sm text-violet-900 shadow-sm transition-all focus:border-violet-400 focus:outline-none focus:ring-4 focus:ring-violet-100"
      />
    </div>
  );
}

function TextField({
  label,
  value,
  onChange,
  placeholder,
  mono,
}: {
  label: string;
  value: string;
  onChange: (v: string) => void;
  placeholder?: string;
  mono?: boolean;
}) {
  return (
    <div>
      <label className="mb-1.5 block text-xs font-semibold text-violet-500">{label}</label>
      <input
        type="text"
        value={value}
        placeholder={placeholder}
        onChange={(e) => onChange(e.target.value)}
        className={`w-full rounded-xl border border-violet-100 bg-white/80 px-3.5 py-2.5 text-sm text-violet-900 shadow-sm transition-all placeholder:text-violet-300 focus:border-violet-400 focus:outline-none focus:ring-4 focus:ring-violet-100 ${
          mono ? "font-mono" : ""
        }`}
      />
    </div>
  );
}

function Info({ label, value, full }: { label: string; value: string | null; full?: boolean }) {
  return (
    <div className={full ? "sm:col-span-2" : ""}>
      <p className="text-xs font-semibold text-violet-400">{label}</p>
      <p className="text-sm text-violet-900">{value || "—"}</p>
    </div>
  );
}
